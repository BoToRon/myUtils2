...
_
import {
	compareArrays, divine, fsReadFileAsync, getEnviromentVariables, getZodSchemaFromData,
	importFileFromProject, messageHandler, successLog, zMyEnv, zodCheck_curry, zRegexGenerator
} from './btr.js'
_
...
const errorHandler = divine.error as messageHandler
...
function checkServerAndClientFilesLogTheirInitialization() {
	...
	[serverTsFiles, clientTsFiles, clientVueFiles].flat(1).forEach(file => {
		const { filename, content } = file
		if (!content.includes(`logInitialization(${filename})`)) { addToErrors(`Initialization of '${filename}' should be logged`) }
	})
}
