 ...
 * (?<=export const \w{1,}) =
 * REPLACE export const WITH async
...
/**
	 ...
	 */
export async function retryF<F extends (...args: any) => any>(
	fn: F,
	args: Parameters<F>,
	retriesLeft: number,
...
export const pipe_persistentType = <T>(initialValue: T, ...fns: typePersistingFn<T>[]) => {
	return fns.reduce((result, fn) => fn(result), initialValue)
}

/**Pipe with schema validation and error tracking */
export const zPipe = <T>(zSchema: zSchema<T>, initialValue: T, ...fns: typePersistingFn<T>[]) => {
...
export const { delay, isEven, isOdd, isWithinRange, roll, timeStampToDate, toOrdinal } = forNumbers
export const { newToast_client_curry, trackVueComponent_curry, zodCheck_curry } = curries
export const { colorLog, errorLog, isGuest, successLog, toSingleLine } = forStrings
export const { copyToClipboard, downloadFile_client } = forClientOnly
export const { timer_add, timer_kill } = forSetIntervals
export const { deepClone, stringify } = forObjectLikes
export const { doNothing, nullAs } = misc
