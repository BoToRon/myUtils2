...
_;
_;
import { PACKAGE_DOT_JSON, utilsRepoName, zValidNpmCommand_package, zValidNpmCommand_project, zValidVersionIncrement } from './constants/constants.js';
_;
import { checkCodeThatCouldBeUpdated, colorLog, command_package, command_project, delay, divine, fsReadFileAsync, fsWriteFileAsync, getCachedFiles, getEnviromentVariables, killProcess, prompCommitMessageAndPush, questionAsPromise, selfFilter, successLog, zodCheckAndHandle, zodCheck_curry } from './btr.js';
_;
if (command_package) {
    zodCheckAndHandle(zValidNpmCommand_package, command_package, npmRun_package, [command_package], divine.error, true);
}
if (command_project) {
    zodCheckAndHandle(zValidNpmCommand_project, command_project, npmRun_project, [command_project], divine.error, true);
}
...
export function npmRun_package(npmCommand) {
    ...
    async function promptVersioning() {
        ...
        if (!zodCheck_curry(tryAgain, true)(zValidVersionIncrement, versionIncrement)) {
            return;
        }
