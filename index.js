...
export const colorLog_big = (variant, message) => {
	clientOrServer_screener('server', () => {
		const log = (message) => colorLog(variant, message, true);
		const logAsterisks = (lines) => {
...
export const colorLog = async (variant, message, bold) => {
	const colors = {
		primary: 'blue',
		secondary: 'grey',
		success: 'green',
...
export const getMainDependencies = async (appName, packageJson, pingMeOnErrors, erisToken, mongoUri, port) => {
	...
	async function getDivineBotAndError() {
		...
		function connectToDiscord() {
			...
			async function connectToDiscord() {
				try {
					divineBot.connect().then(() => colorLog('success', `${divinePrepend} Connected successfully^^`, false));
				}
				catch {
	...
	async function myUtils_checkIfUpToDate(failureHandler) {
		const latestVersion = await getVersionOfJsonPackage();
		const installedVersion = (await import('./package.json', { assert: { type: "json" } })).default.version;
		if (installedVersion === latestVersion) {
			colorLog('info', '@botoron/my-utils is up to date 👍', false);
			return;
		}
		const failureMessage = toSingleLine(`
		...
		async function getVersionOfJsonPackage() {
			const packageJsonFromGithub = await getLatestPackageJsonFromGithub();
			const matchForVersion = packageJsonFromGithub.match(/(?<="version": ")([0-9]|\.){1,}/) || ['none'];
			const latestVersion = matchForVersion[0];
...
export const npmRun = async (npmCommand) => {
	...
	async function promptVersioning() {
		...
		async function getToBeVersionAndUseinChangelongAndPackageJson() {
			const regexOldVersion_packageJson = /([0-9]|\.){1,}/;
			const regexOldVersion_changelong = /#{1,}/;
			const nextVersion = await getNextVersion(regexOldVersion_packageJson);
			await replaceVersionInFile('package.json', regexOldVersion_packageJson, nextVersion);
			await replaceVersionInFile('changelog.MD', regexOldVersion_changelong, nextVersion);
			async function getNextVersion(versionRegex) {
				const packageJson = await fs.promises.readFile('./package.json', 'utf8');
				const versionNumber = (packageJson.match(versionRegex) || ['0'])[0];
				const versionIndex = ['major', 'minor', 'patch'].indexOf(versionIncrement);
				return versionNumber.split('.').map(x => Number(x)).map((x, i, s) => {
					if (i === versionIndex) {
						return x + 1;
					}
					if (i < versionIndex) {
			...
			async function replaceVersionInFile(filename, regexOldVersion, nextVersion) {
				const filePath = './' + filename;
				const file = await fs.promises.readFile(filePath, 'utf8');
				const newFile = file.replace(regexOldVersion, nextVersion);
